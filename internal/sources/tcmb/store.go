...
func (s Store) PersistRecords(ctx context.Context, seriesID string, records []ingestion.NormalizedRecord) (int64, error) {
	...
	for index, normalized := range records {
		...
		if decoded.SeriesCode != seriesID {
			continue
		}
...
// canonical base/quote contract for every FX series.
func (s Store) PersistFXRecords(ctx context.Context, baseCurrency, quoteCurrency string, records []ingestion.NormalizedRecord) (int64, error) {
	if s.Pool == nil {
		return 0, errors.New("TCMB EVDS database pool is required")
	}
